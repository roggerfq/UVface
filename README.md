...
If the container is executed inside a Linux system running on WSL2, it will not have access to USB cameras. In this case, it is necessary to run the Python script (`stream.py`)[https://github.com/roggerfq/UVface/tree/main/stream] on windows to capture video from the camera and send it to `http://172.20.16.1:5000/video_feed`:
